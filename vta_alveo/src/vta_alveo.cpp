...
#pragma HLS INTERFACE m_axi port = insns offset = slave bundle = insns_port
...
#pragma HLS INTERFACE s_axilite port = return bundle = control
  ...
  static inp_vec_T inp_mem[VTA_INP_BUFF_DEPTH][VTA_BATCH];     // inp_vec_T is VTA_BLOCK_IN wide
  static wgt_vec_T wgt_mem[VTA_WGT_BUFF_DEPTH][VTA_BLOCK_OUT]; // wgt_vec_T is VTA_BLOCK_IN wide

  static acc_vec_T acc_mem[VTA_ACC_BUFF_DEPTH][VTA_BATCH];     // acc_vec_T is VTA_BLOCK_OUT wide
//  static acc_vec_T acc_mem[1024];
...
//                 size * sizeof(acc_vec_T) * VTA_BATCH);
          for (int t = 0; t < size * VTA_BATCH; t++)  {
            acc_mem[sram_base+t][0] = biases[(dram_base +t)*VTA_BATCH];
          }
        }
      } else {
...
//               size * sizeof(acc_vec_T) * VTA_BATCH);
          for (int t = 0; t < size * VTA_BATCH; t++)  {
            outputs[dram_base+t*VTA_BATCH] = acc_mem[sram_base+t][0]; // this is the next!!!
          }
