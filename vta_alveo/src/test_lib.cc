...
//  OCL_CTX_p->q.enqueueBarrier();
  ...
  std::cout << "Wall Clock Time (write execution): " << write_duration << std::endl;
...
//  T **array = static_cast<T **>(malloc(sizeof(T *) * rows));
  void *a, *b;
  posix_memalign((void**)&a, 4096, sizeof(T *) * rows);
  T **array = static_cast<T **>(a);
  for (int i = 0; i < rows; i++) {
//    array[i] = static_cast<T *>(malloc(sizeof(T) * cols));
    posix_memalign((void**)&b, 4096, sizeof(T) * cols);
    array[i] = static_cast<T *>(b);
...
//  return memalign(64, num_bytes);
  void *a;
  posix_memalign((void**)&a, 4096, num_bytes);
  return a;
  //return (void*)NULL;
#else
...
#else
std::vector<VTAGenericUop, aligned_allocator<VTAGenericUop>> getResetUops(int batch, int out_feat) {
#endif
  // Converter
...
//  std::vector<VTAGenericInsn, aligned_allocator<VTAGenericInsn>> insn_buf(ins_size);
  std::vector<VTAGenericUop, aligned_allocator<VTAGenericUop>> uop_buf(uop_size);
#endif
  // Generate micro ops
  int uop_idx = 0;
...
#else
std::vector<VTAGenericUop, aligned_allocator<VTAGenericUop>> getGEMMUops(int batch, int in_feat, int out_feat) {
#endif
  // Converter
  union VTAUop converter;
  // Derive the total uop size
...
//  }
  ...
  pack2dBuffer<acc_T, VTA_ACC_WIDTH>(input_buf,
                                     ...
                                     out_channels,
                                     VTA_BATCH,
                                     // XXX ISS, this seems to be VTA_BLOCK_IN <<< 
                                     VTA_BLOCK_OUT);
  ...
  std::vector<inp_T, aligned_allocator<inp_T>> inputs_buf(VTA_ACC_ELEM_BYTES * xfer_size/sizeof(inp_T));
...
//  std::vector<acc_T, aligned_allocator<acc_T>> biases_buf(xfer_size);
  std::vector<acc_T, aligned_allocator<acc_T>> biases_buf(xfer_size*VTA_ACC_ELEM_BYTES/sizeof(acc_T));

//  std::vector<acc_T, aligned_allocator<acc_T>> output_buf(xfer_size);
  std::vector<acc_T, aligned_allocator<acc_T>> output_buf(xfer_size*VTA_ACC_ELEM_BYTES/sizeof(acc_T));
...
//    biases_buf[i] = i;
//    output_buf[i] = 0;
//    printf("%s%d bias[%d] 0x%x\n", __func__, __LINE__, i, biases[0][i]);
...
int reset_test(int batch, int out_channels) {
...
#else
  std::vector<VTAGenericUop, aligned_allocator<VTAGenericUop>> uops_buf = getResetUops(batch / VTA_BATCH, out_channels / VTA_BLOCK_OUT);
 ...
 #else
  printInstruction(ins_size, &insn_buf[0]);
  printMicroOp(uop_size, &uops_buf[0]);
 #endif // NO_SIM
  ...
  // Initialize input data
  inp_T **inputs = allocInit2dArray<inp_T, VTA_INP_WIDTH>(batch, out_channels);
  ...
  acc_T *output_buf = static_cast<acc_T *>(allocBuffer(VTA_ACC_ELEM_BYTES * xfer_size));
#else
  std::vector<inp_T, aligned_allocator<inp_T>> inputs_buf(VTA_INP_ELEM_BYTES * xfer_size/sizeof(inp_T));
  pack2dBuffer<inp_T, VTA_INP_WIDTH>(&inputs_buf[0],
                                     inputs,
                                     batch,
                                     out_channels,
                                     VTA_BATCH,
...
int fc_test(int batch, int in_channels, int out_channels) {
...
#else
  std::vector<VTAGenericUop, aligned_allocator<VTAGenericUop>> uops_buf = getGEMMUops(batch / VTA_BATCH,
                                                                                      in_channels / VTA_BLOCK_IN,
                                                                                      out_channels / VTA_BLOCK_OUT);
#endif
...
#else
  // Prepare the input buffer
  std::vector<inp_T, aligned_allocator<inp_T>> inputs_buf(VTA_INP_ELEM_BYTES * inp_size/sizeof(inp_T));
  pack2dBuffer<inp_T, VTA_INP_WIDTH>(&inputs_buf[0],
                                     inputs,
                                     batch,
                                     out_channels,
                                     ...
                                     VTA_BLOCK_OUT);
  // Prepare the weight buffer
  std::vector<wgt_T, aligned_allocator<wgt_T>> weights_buf(VTA_WGT_ELEM_BYTES*wgt_size/sizeof(wgt_T));
  pack2dBuffer<wgt_T, VTA_WGT_WIDTH>(&weights_buf[0],
                                     weights,
                                     out_channels,
                                     in_channels,
                                     VTA_BLOCK_OUT,
                                     VTA_BLOCK_IN);
  // Prepare the bias buffer
  std::vector<acc_T, aligned_allocator<acc_T>> biases_buf(VTA_ACC_ELEM_BYTES*out_size/sizeof(acc_T));
  pack2dBuffer<acc_T, VTA_ACC_WIDTH>(&biases_buf[0],
                                     biases,
                                     batch,
                                     out_channels,
                                     VTA_BATCH,
                                     VTA_BLOCK_OUT);
  // Prepare the output buffer
  std::vector<acc_T, aligned_allocator<acc_T>> output_buf(VTA_ACC_ELEM_BYTES*out_size/sizeof(acc_T));
#endif
  ...
  if (err == 0) {
    ...
    printf("INFO - FC test failed, got %d errors/%d!\n", err, total);
    return -1;
  }
}
